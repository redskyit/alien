...
Alien was developed to overcome a limitation fo `ab(1)` (Apache Benchmark) so that each request is generated so that they are not all the same. It was developed for a specific task and as such it lacks certain functionality out of the box. For example, the default module only supports `GET` requests.
...
The executables are bundled nodejs runtime and the javascript code generated by [pkg](https://www.npmjs.com/package/pkg).

    git clone https://github.com/redskyit/alien
    cd alien
    npm run build

On successful build the `dist` folder contains the built executables as well as a minified alien.js for use with node.
